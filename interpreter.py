...
def check_python_blocks(raw_lines):
    """Enforces netch's rule for mixing in raw Python:
    - every <python> block needs a '# why: ...' comment right above it explaining the reason
    - raw Python can never be more than 30% of the file (netch-majority rule)
    ...
    while i < len(raw_lines):
        ...
        if stripped == '<python>':
            ...
            if not has_reason:
                print(f"[netch warning] the <python> block at line {i + 1} is missing a '# why: ...' comment "
                      f"right above it explaining why you need raw Python here instead of netch. "
            ...
            while k < len(raw_lines):
                ...
                k += 1
            if not closed:
                print(f"[netch error] the <python> block starting at line {i + 1} is missing its closing </python> tag.")
                return False
    ...
    if total_code_lines > 0 and (python_lines / total_code_lines) > 0.30:
        percent = round((python_lines / total_code_lines) * 100)
        print(f"[netch warning] this file is {percent}% raw Python — netch scripts have to stay mostly netch "
              f"(raw Python can only make up under 30% of the file). Move some of that logic into real netch, "
              f"or trim down the <python> blocks, then run it again.")
        return False
...
def check_for_update():
    ...
    try:
        ...
        print("[netch] updated! restarting with the new version...")
        os.execv(sys.executable, [sys.executable] + sys.argv)  # relaunches automatically with the fresh file
        ...
        for stmt in statements:
            ...
            if wbutton_bare_match:
                ...
                btn = tk.Button(root, text=btn_name, font=("Segoe UI", 13), bg="#4a90d9", fg="white",
                                 ...
                                 command=(lambda n=btn_name: window_state["button_clicked"].update({n: True})))
                btn.pack(pady=8)
            ...
            if wbutton_match:
                ...
                def make_button_command(name, action, s):
                    def on_click():
                        window_state["button_clicked"][name] = True
                        if action:
                            execute([{"type": "line", "text": action}], s)
                    return on_click

                btn = tk.Button(root, text=btn_name, font=("Segoe UI", 13), bg="#4a90d9", fg="white",
                                 activebackground="#3a7bc8", activeforeground="white",
                                 relief="flat", borderwidth=0, padx=16, pady=8, cursor="hand2",
                                 command=make_button_command(btn_name, action_text, scope))
                btn.pack(pady=8)
                window_state["buttons"][btn_name] = btn
                root.update()
                continue
            ...
            if bposition_match:
                ...
                if btn_name not in window_state["buttons"]:
                    print(f"[netch error] button '{btn_name}' doesn't exist yet")
                    continue
                widget = window_state["buttons"][btn_name]
                widget.pack_forget()
