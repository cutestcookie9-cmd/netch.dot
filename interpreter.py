...
def run_netch(filepath):
    ...
    window_state = {"root": None, "widgets": [], "buttons": {}, "selections": {}, "textboxes": {}, "button_clicked": {},
                     ...
                     "theme": {"bg": "#f4f5f7", "fg": "#2d2d2d", "accent": "#5865F2",
                               "textbox_bg": "white", "textbox_fg": "#2d2d2d"},
                     "key_pressed": {}, "checkboxes": {}, "sliders": {}, "progressbars": {}, "images": {}}
    ...
    def resolve_value(token, scope):
        token = token.strip()
        random_match = re.match(r'^random\((.+?)\s*,\s*(.+)\)$', token)
        if random_match:
            lo = resolve_value(random_match.group(1), scope)
            hi = resolve_value(random_match.group(2), scope)
            try:
                return random_module.randint(int(lo), int(hi))
            except (ValueError, TypeError):
                print(f"[netch error] random({lo}, {hi}) needs two whole numbers")
                return 0
        ...
        cb_match = re.match(r'^checkbox\.(\w+)$', token)
        if cb_match and cb_match.group(1) in window_state["checkboxes"]:
            return window_state["checkboxes"][cb_match.group(1)]["var"].get()
        slider_match = re.match(r'^slider\.(\w+)$', token)
        if slider_match and slider_match.group(1) in window_state["sliders"]:
            return window_state["sliders"][slider_match.group(1)].get()
    ...
    def evaluate_condition(cond, scope):
        ...
        keypress_match = re.match(r'^key\.pressed\((.+)\)$', cond)
        if keypress_match:
            key_name = keypress_match.group(1).strip().strip('"').lower()
            was_pressed = window_state["key_pressed"].get(key_name, False)
            window_state["key_pressed"][key_name] = False  # consume so it only fires once per press
            return was_pressed
    ...
    def render_progress(name):
        info = window_state["progressbars"][name]
        cv, geo = info["canvas"], info["geo"]
        cv.delete("all")
        theme = window_state["theme"]
        pct = max(0, min(1, geo["value"] / geo["max"])) if geo["max"] else 0
        cv.create_rectangle(0, 0, geo["w"], geo["h"], fill=theme["textbox_bg"], outline=theme["textbox_bg"])
        cv.create_rectangle(0, 0, geo["w"] * pct, geo["h"], fill=theme["accent"], outline=theme["accent"])
    ...
    def get_window(state):
        if state["root"] is None:
            ...
            state["root"].configure(bg=state["theme"]["bg"])
            ...
            def on_key_press(event):
                state["key_pressed"][event.keysym.lower()] = True

            state["root"].bind("<KeyPress>", on_key_press)
        return state["root"]
    ...
    def execute(statements, scope):
        for stmt in statements:
            ...
            wtheme_match = re.match(r'^window\.theme\((.+)\)$', stripped)
            if wtheme_match:
                if not window_mode:
                    print("[netch error] window.theme() only works in a window app (add <window.using> under <using.netch>)")
                    continue
                choice = wtheme_match.group(1).strip().strip('"').lower()
                if choice == 'dark':
                    window_state["theme"] = {"bg": "#1e1f24", "fg": "#e8e8ee", "accent": "#5865F2",
                                              "textbox_bg": "#2a2b32", "textbox_fg": "#e8e8ee"}
                elif choice == 'light':
                    window_state["theme"] = {"bg": "#f4f5f7", "fg": "#2d2d2d", "accent": "#4a90d9",
                                              "textbox_bg": "white", "textbox_fg": "#2d2d2d"}
                elif re.match(r'^#[0-9a-fA-F]{6}$', choice):
                    r, g, b = int(choice[1:3], 16), int(choice[3:5], 16), int(choice[5:7], 16)
                    luminance = 0.299 * r + 0.587 * g + 0.114 * b
                    fg = "#2d2d2d" if luminance > 140 else "#e8e8ee"
                    box_bg = "white" if luminance > 140 else "#2a2b32"
                    window_state["theme"] = {"bg": choice, "fg": fg, "accent": "#5865F2",
                                              "textbox_bg": box_bg, "textbox_fg": fg}
                else:
                    print(f"[netch error] window.theme(\"{choice}\") isn't recognized — use \"dark\", \"light\", or a hex color like \"#1e1f24\"")
                    continue
                get_window(window_state).configure(bg=window_state["theme"]["bg"])
                continue
            ...
            if wtext_match:
                ...
                root = get_window(window_state)
                theme = window_state["theme"]
                label = tk.Label(root, text=text, font=("Segoe UI", 20), bg=theme["bg"], fg=theme["fg"])
            ...
            if wtextbox_match:
                ...
                box = tk.Text(root, height=1, width=10, font=("Segoe UI", 13), wrap="word", relief="flat",
                              bg=theme["textbox_bg"], fg=theme["textbox_fg"], insertbackground=theme["textbox_fg"],
            ...
            if wselect_match:
                ...
                if sel_type == 'dropdown':
                    ...
                    widget.config(font=("Segoe UI", 12), bg=theme["textbox_bg"], fg=theme["textbox_fg"],
                    ...
                    for opt in options:
                        rb = tk.Radiobutton(frame, text=opt, variable=var, value=opt,
                                             ...
                                             activebackground=theme["bg"], selectcolor=theme["textbox_bg"], cursor="hand2")
            ...
            wcheckbox_match = re.match(r'^window\.checkbox\.(\w+)\s*=\s*(.+)$', stripped)
            if wcheckbox_match:
                if not window_mode:
                    print("[netch error] window.checkbox only works in a window app (add <window.using> under <using.netch>)")
                    continue
                cb_name, label_text = wcheckbox_match.groups()
                label_text = label_text.strip().strip('"')
                root = get_window(window_state)
                theme = window_state["theme"]
                var = tk.BooleanVar(root)
                cb = tk.Checkbutton(root, text=label_text, variable=var, font=("Segoe UI", 12),
                                     bg=theme["bg"], fg=theme["fg"], activebackground=theme["bg"],
                                     selectcolor=theme["textbox_bg"], cursor="hand2")
                cb.place(x=20, y=20 + 40 * len(window_state["checkboxes"]))
                window_state["checkboxes"][cb_name] = {"var": var, "widget": cb}
                root.update()
                continue

            cbposition_match = re.match(r'^checkbox\.(\w+)\.position\((\d+)\s*,\s*(\d+)\)$', stripped)
            if cbposition_match:
                cb_name, x, y = cbposition_match.groups()
                if cb_name not in window_state["checkboxes"]:
                    print(f"[netch error] checkbox '{cb_name}' doesn't exist yet")
                    continue
                window_state["checkboxes"][cb_name]["widget"].place(x=int(x), y=int(y))
                window_state["root"].update()
                continue

            wslider_match = re.match(r'^window\.slider\.(\w+)\s*=\s*slider\((.+?)\s*,\s*(.+)\)$', stripped)
            if wslider_match:
                if not window_mode:
                    print("[netch error] window.slider only works in a window app (add <window.using> under <using.netch>)")
                    continue
                slider_name, lo, hi = wslider_match.groups()
                try:
                    lo_val, hi_val = int(lo), int(hi)
                except ValueError:
                    print(f"[netch error] slider(...) needs two whole numbers, got ({lo}, {hi})")
                    continue
                root = get_window(window_state)
                theme = window_state["theme"]
                sl = tk.Scale(root, from_=lo_val, to=hi_val, orient="horizontal",
                              bg=theme["bg"], fg=theme["fg"], troughcolor=theme["textbox_bg"],
                              highlightthickness=0, font=("Segoe UI", 10))
                sl.place(x=20, y=20 + 60 * len(window_state["sliders"]), width=200)
                window_state["sliders"][slider_name] = sl
                root.update()
                continue

            sliderposition_match = re.match(r'^slider\.(\w+)\.position\((\d+)\s*,\s*(\d+)\)$', stripped)
            if sliderposition_match:
                slider_name, x, y = sliderposition_match.groups()
                if slider_name not in window_state["sliders"]:
                    print(f"[netch error] slider '{slider_name}' doesn't exist yet")
                    continue
                window_state["sliders"][slider_name].place(x=int(x), y=int(y))
                window_state["root"].update()
                continue

            slidersize_match = re.match(r'^slider\.(\w+)\.size\((\d+)\s*,\s*(\d+)\)$', stripped)
            if slidersize_match:
                slider_name, w, h = slidersize_match.groups()
                if slider_name not in window_state["sliders"]:
                    print(f"[netch error] slider '{slider_name}' doesn't exist yet")
                    continue
                window_state["sliders"][slider_name].place(width=int(w), height=int(h))
                window_state["root"].update()
                continue

            wprogress_match = re.match(r'^window\.progress\.(\w+)\s*=\s*progress\((.+)\)$', stripped)
            if wprogress_match:
                if not window_mode:
                    print("[netch error] window.progress only works in a window app (add <window.using> under <using.netch>)")
                    continue
                progress_name, max_val = wprogress_match.groups()
                try:
                    max_int = int(max_val.strip())
                except ValueError:
                    print(f"[netch error] progress(...) needs a whole number, got {max_val}")
                    continue
                root = get_window(window_state)
                theme = window_state["theme"]
                geo = {"x": 20, "y": 20 + 40 * len(window_state["progressbars"]), "w": 200, "h": 18, "max": max_int, "value": 0}
                cv = tk.Canvas(root, width=geo["w"], height=geo["h"], bg=theme["textbox_bg"], highlightthickness=0)
                cv.place(x=geo["x"], y=geo["y"])
                window_state["progressbars"][progress_name] = {"canvas": cv, "geo": geo}
                render_progress(progress_name)
                root.update()
                continue

            progressset_match = re.match(r'^progress\.(\w+)\.set\((.+)\)$', stripped)
            if progressset_match:
                progress_name, val_expr = progressset_match.groups()
                if progress_name not in window_state["progressbars"]:
                    print(f"[netch error] progress '{progress_name}' doesn't exist yet")
                    continue
                try:
                    val = int(resolve_value(val_expr.strip(), scope))
                except (ValueError, TypeError):
                    print(f"[netch error] progress.{progress_name}.set(...) needs a whole number")
                    continue
                window_state["progressbars"][progress_name]["geo"]["value"] = val
                render_progress(progress_name)
                window_state["root"].update()
                continue

            progressposition_match = re.match(r'^progress\.(\w+)\.position\((\d+)\s*,\s*(\d+)\)$', stripped)
            if progressposition_match:
                progress_name, x, y = progressposition_match.groups()
                if progress_name not in window_state["progressbars"]:
                    print(f"[netch error] progress '{progress_name}' doesn't exist yet")
                    continue
                geo = window_state["progressbars"][progress_name]["geo"]
                geo["x"], geo["y"] = int(x), int(y)
                window_state["progressbars"][progress_name]["canvas"].place(x=geo["x"], y=geo["y"])
                window_state["root"].update()
                continue

            progresssize_match = re.match(r'^progress\.(\w+)\.size\((\d+)\s*,\s*(\d+)\)$', stripped)
            if progresssize_match:
                progress_name, w, h = progresssize_match.groups()
                if progress_name not in window_state["progressbars"]:
                    print(f"[netch error] progress '{progress_name}' doesn't exist yet")
                    continue
                geo = window_state["progressbars"][progress_name]["geo"]
                geo["w"], geo["h"] = int(w), int(h)
                window_state["progressbars"][progress_name]["canvas"].config(width=geo["w"], height=geo["h"])
                render_progress(progress_name)
                window_state["root"].update()
                continue

            wimage_match = re.match(r'^window\.image\.(\w+)\s*=\s*(.+)$', stripped)
            if wimage_match:
                if not window_mode:
                    print("[netch error] window.image only works in a window app (add <window.using> under <using.netch>)")
                    continue
                image_name, path = wimage_match.groups()
                path = path.strip().strip('"')
                root = get_window(window_state)
                try:
                    pil = ensure_installed("PIL", "pillow")
                    if pil:
                    ...
                    label = tk.Label(root, image=tk_img, bd=0)
                    label.image = tk_img  # keep a reference so it isn't garbage collected
                    label.place(x=20, y=20 + 40 * len(window_state["images"]))
                    window_state["images"][image_name] = {"widget": label, "path": path}
                    root.update()
                except Exception as e:
                    print(f"[netch error] couldn't load image '{path}': {e}")
                continue

            imgposition_match = re.match(r'^image\.(\w+)\.position\((\d+)\s*,\s*(\d+)\)$', stripped)
            if imgposition_match:
                image_name, x, y = imgposition_match.groups()
                if image_name not in window_state["images"]:
                    print(f"[netch error] image '{image_name}' doesn't exist yet")
                    continue
                window_state["images"][image_name]["widget"].place(x=int(x), y=int(y))
                window_state["root"].update()
                continue

            imgsize_match = re.match(r'^image\.(\w+)\.size\((\d+)\s*,\s*(\d+)\)$', stripped)
            if imgsize_match:
                image_name, w, h = imgsize_match.groups()
                if image_name not in window_state["images"]:
                    print(f"[netch error] image '{image_name}' doesn't exist yet")
                    continue
                info = window_state["images"][image_name]
                try:
                    pil = ensure_installed("PIL", "pillow")
                    root = get_window(window_state)
                    if pil:
                        from PIL import Image, ImageTk
                        pil_img = Image.open(info["path"]).resize((int(w), int(h)))
                        tk_img = ImageTk.PhotoImage(pil_img)
                        info["widget"].config(image=tk_img)
                        info["widget"].image = tk_img
                    else:
                        print("[netch warning] resizing images needs Pillow — showing it at its original size instead")
                except Exception as e:
                    print(f"[netch error] couldn't resize image '{image_name}': {e}")
                window_state["root"].update()
                continue
            ...
            if wbutton_bare_match:
                ...
                window_state["button_geo"][btn_name] = {
                    ...
                    "color": window_state["theme"]["accent"], "radius": 0, "font_size": 13,
            ...
            if wtextlabel_match:
                ...
                if label_name in window_state["labels"]:
                    ...
                    root = get_window(window_state)
                    theme = window_state["theme"]
                    label = tk.Label(root, text=text, font=("Segoe UI", 16), bg=theme["bg"], fg=theme["fg"])
            ...
            downloadfile_match = re.match(r'^download\.file\((.+)\)$', stripped)
            if downloadfile_match:
                args_str = downloadfile_match.group(1)
                parts = [p.strip() for p in args_str.split(',', 1)]
                url = resolve_value(parts[0], scope)
                url = url[1:-1] if isinstance(url, str) and url.startswith('"') and url.endswith('"') else str(url)
                if not re.match(r'^[a-zA-Z][a-zA-Z0-9+.\-]*://', url):
                    url = "https://" + url
                if len(parts) == 2:
                    save_as = resolve_value(parts[1], scope)
                    save_as = save_as[1:-1] if isinstance(save_as, str) and save_as.startswith('"') and save_as.endswith('"') else str(save_as)
                else:
                    save_as = os.path.basename(url.split('?')[0]) or "downloaded_file"
                try:
                    req = urllib.request.Request(url, headers={"User-Agent": "netch"})
                    with urllib.request.urlopen(req, timeout=15) as resp:
                        data = resp.read()
                    with open(save_as, "wb") as f:
                        f.write(data)
                    print(f"[netch] downloaded {url} -> {save_as}")
                except Exception as e:
                    print(f"[netch error] couldn't download '{url}': {e}")
                continue
            ...
            if print_match:
                ...
                cb_read_match = re.match(r'^checkbox\.(\w+)$', content)
                slider_read_match = re.match(r'^slider\.(\w+)$', content)
                length_match = re.match(r'^(\w+)\.length$', content)
                random_match = re.match(r'^random\(.+\)$', content)
                math_result = try_math(content, scope)
                if content.startswith('"') and content.endswith('"'):
                    print(content[1:-1])
                elif random_match:
                    print(resolve_value(content, scope))
                elif cb_read_match and cb_read_match.group(1) in window_state["checkboxes"]:
                    print(window_state["checkboxes"][cb_read_match.group(1)]["var"].get())
                elif slider_read_match and slider_read_match.group(1) in window_state["sliders"]:
                    print(window_state["sliders"][slider_read_match.group(1)].get())
            ...
            if assign_match:
                ...
                random_match = re.match(r'^random\(.+\)$', raw_value)
                ...
                elif re.match(r'^checkbox\.(\w+)$', raw_value) and re.match(r'^checkbox\.(\w+)$', raw_value).group(1) in window_state["checkboxes"]:
                    scope[var_name] = window_state["checkboxes"][re.match(r'^checkbox\.(\w+)$', raw_value).group(1)]["var"].get()
                elif re.match(r'^slider\.(\w+)$', raw_value) and re.match(r'^slider\.(\w+)$', raw_value).group(1) in window_state["sliders"]:
                    scope[var_name] = window_state["sliders"][re.match(r'^slider\.(\w+)$', raw_value).group(1)].get()
